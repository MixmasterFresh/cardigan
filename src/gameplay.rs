    ...
    app.add_systems(OnEnter(GameState::Playing), (setup_gameplay, setup_play_areas).chain())
        ...
        .add_systems(
            ...
            (
                ...
                rebuild_slots_on_resize,           // Rebuild slots when window changes
                card_resize_system,                // Resize cards when window changes
                reposition_played_cards_on_resize, // Reposition cards in play areas
...
// System to update play area slot positions on window resize
// System to rebuild play area slots when window is resized
// This despawns and recreates all slots with correct sizing and dashed borders
pub fn rebuild_slots_on_resize(
    mut commands: Commands,
    slot_query: Query<(Entity, &Children), With<CardSlot>>,
    window_dims: Res<WindowDimensions>,
    config: Res<PlayAreaConfig>,
    gameplay_state: Res<GameplayState>,
) {
    if !window_dims.is_changed() {
        return;
    }

    // Despawn all existing slots (including their children - the dashed borders)
    for (entity, children) in slot_query.iter() {
        // Despawn all children (dashed border sprites)
        for child in children.iter() {
            commands.entity(child).despawn();
        }
        // Despawn the slot entity itself
        commands.entity(entity).despawn();
    }

    let layout = LayoutZones::new(&window_dims);

    // Respawn player play area slots
    let player_y = layout.player_play_area_y(&window_dims);
    let player_positions = layout.calculate_slot_positions(config.player_slots_per_row, player_y);

    for (slot_index, position) in player_positions.iter().enumerate() {
        let occupied = gameplay_state.is_slot_occupied(slot_index);
        spawn_card_slot(
            &mut commands,
            CardZone::PlayerPlayArea { slot: slot_index },
            *position,
    ...
    // Respawn opponent play area slots
    let opponent_y = layout.opponent_play_area_y(&window_dims);
    let opponent_positions = layout.calculate_slot_positions(config.opponent_slots_per_row, opponent_y);

    for (slot_index, position) in opponent_positions.iter().enumerate() {
        // For opponent slots, check the opponent_play_area array
        let occupied = gameplay_state.opponent_play_area[slot_index].is_some();
        spawn_card_slot(
            &mut commands,
            CardZone::OpponentPlayArea { slot: slot_index },
            *position,
...
// System to resize cards and deck when window dimensions change
pub fn card_resize_system(
    mut card_query: Query<(&mut Sprite, &mut Card)>,
    mut deck_query: Query<&mut Sprite, (With<Deck>, Without<Card>)>,
    mut deck_empty_query: Query<&mut Sprite, (With<DeckEmpty>, Without<Card>, Without<Deck>)>,
    mut slot_query: Query<&mut Sprite, (With<CardSlot>, Without<Card>, Without<Deck>, Without<DeckEmpty>)>,
    window_dims: Res<WindowDimensions>,
) {
    if !window_dims.is_changed() {
        return;
    ...
    let layout = LayoutZones::new(&window_dims);
    let new_card_size = layout.card_size;

    // Update all cards
    for (mut sprite, mut card) in card_query.iter_mut() {
        sprite.custom_size = Some(new_card_size);
        card.base_size = new_card_size;
    }

    // Update deck
    for mut sprite in deck_query.iter_mut() {
        sprite.custom_size = Some(new_card_size);
    }

    // Update empty deck placeholder
    for mut sprite in deck_empty_query.iter_mut() {
        sprite.custom_size = Some(new_card_size);
    }

    // Update card slots
    for mut sprite in slot_query.iter_mut() {
        sprite.custom_size = Some(new_card_size);
    }
}

// System to reposition cards in play areas when window resizes
pub fn reposition_played_cards_on_resize(
    mut card_query: Query<(&CardZone, &mut Card), Without<InHand>>,
    window_dims: Res<WindowDimensions>,
    config: Res<PlayAreaConfig>,
) {
    if !window_dims.is_changed() {
    ...
    // Calculate new slot positions for player play area
    let player_y = layout.player_play_area_y(&window_dims);
    let player_positions = layout.calculate_slot_positions(config.player_slots_per_row, player_y);

    // Calculate new slot positions for opponent play area
    let opponent_y = layout.opponent_play_area_y(&window_dims);
    let opponent_positions = layout.calculate_slot_positions(config.opponent_slots_per_row, opponent_y);

    // Update card positions based on their zones
    for (zone, mut card) in card_query.iter_mut() {
        match zone {
            CardZone::PlayerPlayArea { slot } => {
                if let Some(&position) = player_positions.get(*slot) {
                    card.target_position = position;
                }
            }
            CardZone::OpponentPlayArea { slot } => {
                if let Some(&position) = opponent_positions.get(*slot) {
                    card.target_position = position;
                }
            }
            _ => {}
    ...
    if (new_width - window_dims.width).abs() > 0.1 || (new_height - window_dims.height).abs() > 0.1 {
        ...
        for (anchor, mut transform) in anchored_query.iter_mut() {
            match anchor {
                ...
                AnchorPosition::TopCenter { offset_y } => {
                    transform.translation.x = 0.0;
                    transform.translation.y = (new_height / 2.0) + offset_y;
                }
                ...
                AnchorPosition::TopLeft { offset_x, offset_y } => {
                    transform.translation.x = -(new_width / 2.0) + offset_x;
                    transform.translation.y = (new_height / 2.0) + offset_y;
                }
                ...
                AnchorPosition::BottomLeft { offset_x, offset_y } => {
                    transform.translation.x = -(new_width / 2.0) + offset_x;
                    transform.translation.y = -(new_height / 2.0) + offset_y;
                }
            }
